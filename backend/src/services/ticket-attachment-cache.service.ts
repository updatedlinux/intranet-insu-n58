...
const CACHE_PREFIX = 'ticket-att:';
const MAX_CACHE_BYTES = 1024 * 1024;
const CACHE_TTL_SECONDS = 15 * 60;

export interface CachedTicketAttachment {
  buffer: Buffer;
  contentType: string;
  fileName: string;
}
...
export async function getCachedTicketAttachment(
  attachmentId: number,
): Promise<CachedTicketAttachment | null> {
...
export async function setCachedTicketAttachment(
  attachmentId: number,
  payload: CachedTicketAttachment,
): Promise<void> {
